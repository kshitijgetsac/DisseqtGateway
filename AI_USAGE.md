...
- Expanded the suite to 50 tests. The added coverage includes simultaneous idempotency-key races, preserved cancellation history, tied-timestamp keyset pagination, action-ID audit search, JSON Schema email validation, prompt-injection policy impact, scoped provider accounting, representative seed consistency, and revocation of agent permissions after a worker claim but before execution. Worker recovery closes expired attempts and records the lease loss in the audit trail.
