...
export default function Audit() {
  ...
  return <>
    ...
    <form className="filter-bar" onSubmit={submit}><div className="search-control"><Icon name="search"/><input value={filters.q} onChange={e => setFilters({ ...filters, q: e.target.value })} placeholder="Search reason code"/></div><select value={filters.event_type} onChange={e => setFilters({ ...filters, event_type: e.target.value })}><option value="">All event types</option>{['POLICY_EVALUATED','POLICY_REEVALUATED','POLICY_REPLAYED','APPROVAL_REQUESTED','APPROVAL_APPROVED','APPROVAL_REJECTED','AUTHORIZATION_FAILED','JOB_CLAIMED','EXECUTION_SUCCEEDED','EXECUTION_FAILED'].map(x => <option key={x}>{x}</option>)}</select><select value={filters.decision} onChange={e => setFilters({ ...filters, decision: e.target.value })}><option value="">All decisions</option><option>ALLOW</option><option>REQUIRE_APPROVAL</option><option>DENY</option></select><select value={filters.risk} onChange={e => setFilters({ ...filters, risk: e.target.value })}><option value="">All risks</option><option>LOW</option><option>MEDIUM</option><option>HIGH</option></select><button className="button button-primary">Apply filters</button></form>
